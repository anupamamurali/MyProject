...
class StatusReportXls(models.AbstractModel):
    _name = 'report.project_status_level_report.status_report_xls'
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        if date_from and not date_to:
            task = tasks.search([('date_assign', '>=', date_from), ('project_id', '=', project.id)])
        elif date_to and not date_from:
            task = tasks.search([('date_assign', '<=', date_to), ('project_id', '=', project.id)])
        elif date_from and date_to:
            task = tasks.search([('date_assign', '<=', date_to), ('date_assign', '>=', date_from),
     ...
     # ---------------------Bar chart for Time duration--------------------------
        ...
        #     # # ---------------------Bar chart for Budget-------------------------- #
            ...
            for acc in account_obj:
                if acc.account_id == project.analytic_account_id:
            ...
            for data in task:
                ...
                if data.date_end and data.date_end > data.date_assign:
        ...
        #     # ---------------------Gantt chart view-------------------------- #
            ...
            if project.date_start:
                ...
                project_start = datetime.datetime.date(fields.Datetime.from_string(project.create_date))
            start_list = []
            days_list = []
            task_list = []
            task_item = 0
